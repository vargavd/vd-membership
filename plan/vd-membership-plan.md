...
- `Application`: Manages class initializations and plugin activation setups, plus deactivation cleanup (if any). Also, please tests the external db connection on every `admin_init` if the credential options are already filled.
...
- `ugyfel` is the actual ID (primary key), but with no auto increment. The application should handle it manually! For a new member, `ugyfel` should be *max + 1*.
...
- The **edit member** and **new member** page should use the same form template
