...
- `Application`: Manages class initializations, plugin activation checks and testing the external db connection (if the options are already filled), plus deactivation cleanup (if any).
- `MemberService`: Contains the main member-related application logic. Coordinates validation, domain objects, and repository operations for CRUD.
...
- `ugyfel` is the actual ID (primary key), but with no auto increment. The application should handle it manually!
...
- `generalva` should be only readonly, the **INSERT** command should explicitly set to NULL, and the **UPDATE** command should not touch it.
- `esedekes` should be readonly, for new members the field should be null, when updating, it should be preserved. The validation should ignore this field.
- `dij` - no arithmetic will be needed
...
- The project should not use package managers and bundlers (like npm, composer or webpack). Any external JS library should be enqueued from a CDN if possible. Although I am not strict about this, if there are strong counterpoints.
- No unit tests or code analysis for now.
...
- If there is a php or a database error, display it in a notice fully. This tool will be used by only administrators, there is no risk displaying anything.
